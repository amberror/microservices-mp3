...
@Component("kafkaResourceResultConsumer")
@Slf4j
public class KafkaResourceResultConsumer implements ResourceResultConsumer {
	...
	public void consumeMessage(Long key, Long message, String topic) {
		log.info("[RESOURCE-RESULT-CONSUMER] message received [{}]", key);
		messagingService.processResourceResult(message);
	}

	@Override
	public void consumeMessageDlt(Long key, Long message, String topic) {
		log.info("[KAFKA-DEAD-LETTER-RESOURCE-RESULT-CONSUMER] message received key : [{}], message [{}]", key, message);
		try {
